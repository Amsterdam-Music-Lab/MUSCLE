...
<img alt="Question List" src="../assets/images/QuestionList.png">
...
To add questions to the Question List, you can either select from a list of questions, or add all questions from a question bank, defined in the `question/fixtures` directory in `yaml` format. After creating a Question List from a bank, you can remove questions or add other questions as you wish without affecting the original question bank.
...
By checking "Is skippable", you give a participant the option to skip the question.
...
- **Text** will show a text field - use this for open questions (specify maximum length through `Max length` field)

If you select a question type which presents choices to the participant (i.e., all question types besides **Number**, **Range** and **Text**), the following fields will appear:

- Profile scoring rule: for Likert questions, choose how the index of the choice should be converted to a number: first choice has lowest score (LIKERT) or first choice has highest score (REVERSE_LIKERT).
- Choices: you can choose one of the preset choice lists, or add a new one by clicking the `+` button.
