...
class Categorization(Base):
    ...
    def first_round(self, block):
        ...
        experiment_translated_content = block.phase.experiment.get_current_content()
        consent_text = experiment_translated_content.consent
    ...
    def next_round(self, session):
        ...
        get_rounds_passed = session.get_rounds_passed() - int(json_data["training_rounds"])
    ...
    def get_title(self, session):
        json_data = session.load_json_data()
        get_rounds_passed = session.get_rounds_passed() - int(json_data["training_rounds"]) + 1
