...
def next_trial_actions(session, round_number, request_session):
    ...
    if session.final_score == 0:
        ...
        if previous_results.count():
            ...
            actions.append(
                response_explainer(previous_results.first().scoring.value, same)
            )
        if round_number == 5:
            total_score = sum(
                [res.scoring.value for res in previous_results.all()[:4]])
            if total_score < 2:
                # start practice over
...
def finalize_experiment(session, request_session):
    ...
    percentage = (sum([res.scoring.value for res in session.result_set.all()]
