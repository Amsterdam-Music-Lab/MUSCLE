...
class TwoAlternativeForced(BaseRules):
    ...
    def next_round(self, session):
        ...
        Actions used here: Final, Trial (returned by get_profile_questions(), next_trial_action()), Explainer (also returned by get_feedback())
        """

        # get_profile_questions() returns questions that haven't been asked yet
        actions = self.get_profile_questions(session)
        if actions:
            return actions
