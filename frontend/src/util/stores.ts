...
const createDocumentHeadSlice: StateCreator<DocumentHeadSlice> = (set) => ({
  ...
  patchHeadData: (headData) =>
    set((state) => ({ headData: { ...state.headData, ...headData } })),
  resetHeadData: () =>
    set(() => ({
      headData: {
...
const createParticipantSlice: StateCreator<ParticipantSlice> = (set) => ({
  ...
  setParticipantLoading: (participantLoading: boolean) =>
    set(() => ({ participantLoading })),
});
...
const createExperimentSlice: StateCreator<ExperimentSlice> = (set) => ({
  experiment: null,
  setExperiment: (experiment: Experiment) => set(() => ({ experiment })),
});

interface ConsentSlice {
  hasShownConsent: boolean;
  setHasShownConsent: (value: boolean) => void;
}

const createConsentSlice: StateCreator<ConsentSlice> = (set) => ({
  hasShownConsent: false,
  setHasShownConsent: (value: boolean) =>
    set(() => ({ hasShownConsent: value })),
});
