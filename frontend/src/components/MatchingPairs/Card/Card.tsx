...
export interface CardProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Whether the card has been seen — CURRENTLY UNUSED. */
  seen?: boolean;

  /** Whether the card should not respond to events */
  noEvents?: boolean;
...
function Card({
  ...
  // Default Front
  if(!front) {
    front = <Card.Front className={variantFront ? `fill-${variantFront}` : ""}></Card.Front>
  }

  // DefaultBack
  if(!back) {
    back = <Card.Back className={variantBack ? `fill-${variantBack}` : ""}></Card.Back>
  }
...
Card.Back = function CardBack({ seen, className, ...props }: CardBackProps) {
  return <div 
    data-testid="back" 
    className={classNames(styles.back, { seen }, className)} 
    {...props} 
  />
};
