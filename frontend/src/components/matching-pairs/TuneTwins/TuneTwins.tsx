...
const DEFAULT_FEEDBACK: Record<TTFeedbackMessage, string[]> = {
    ...
    completedLuckyMatch: ["Lucky guess!", "Lucky you!", "This is your lucky day!"],
...
export default function TuneTwins({
    ...
    const { activeSteps, feedback, gameState, cards, turnScore, totalScore, endTurn, selectCard } =
        useTuneTwins({
            ...props,
            feedbackMessages,
    ...
    return (
        <SquareLayout portraitHeaderHeight={0.55} className={styles.container} fullscreen={true}>
                ...
                <Board
                    ...
                    items={cards.map((card) => {
                        ...
                        return [
                            ...
                            >
                                <AudioCard key={card.id} running={card.playing} {...sharedProps} />
                            </div>,
...
function TuneTwinsHeader({ turnScore, totalScore, feedback, activeSteps }: TuneTwinsHeaderProps) {
    ...
    return (
        <>
            ...
            {activeSteps && activeSteps.map((step) => <TutorialMessage key={step.id} {...step} />)}
        </>
    );
}
