...
describe("Checkboxes", () => {
  ...
  it("checks boxes based on initial value", () => {
    ...
    expect(
      screen
        ...
        .parentElement.classList.contains(styles.checked)
    ).toBe(true);
    expect(
      screen
        .getByText("Second Option")
        .parentElement.classList.contains(styles.checked)
    ).toBe(false);
    expect(
      screen
        .getByText("Third Option")
        .parentElement.classList.contains(styles.checked)
    ).toBe(true);
  });
  ...
  it("handles keyboard interactions", async () => {
    ...
    firstOption.focus();
    await fireEvent.keyDown(firstOption, {
      key: "Enter",
      code: "Enter",
      charCode: 13,
      keyCode: 13,
  ...
  it("throws an error when no choices are provided", () => {
    ...
    expect(() =>
      render(
        <Checkboxes question={invalidQuestion} value="" onChange={() => {}} />
      )
    ...
    expect(() =>
      render(
        <Checkboxes question={noChoicesQuestion} value="" onChange={() => {}} />
      )
    ).toThrow("Checkboxes question must have choices");
  });
});
